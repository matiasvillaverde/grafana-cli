...
func (a *App) runCloudAccessPolicies(ctx context.Context, opts globalOptions, args []string) error {
	...
	case "list":
		...
		limit := fs.Int("limit", 100, "maximum access policies")
		...
		pageCursor := fs.String("page-cursor", "", "cursor for the first page to fetch")
		...
		result, count, truncated, err := a.listCloudAccessPolicies(ctx, client, grafana.CloudAccessPolicyListRequest{
		...
		return a.emitWithMetadata(opts, result, accessPolicyMetadata(count, truncated))
	case "get":
...
func (a *App) listCloudAccessPolicies(ctx context.Context, client APIClient, req grafana.CloudAccessPolicyListRequest, limit int) (any, int, bool, error) {
	return a.listCloudCollection(ctx, func(ctx context.Context, pageSize int, pageCursor string) (any, error) {
		...
		pageReq.PageCursor = pageCursor
		return client.CloudAccessPolicies(ctx, pageReq)
	}, cloudListOptions{
		Limit:    limit,
...
func accessPolicyMetadata(count int, truncated bool) *responseMetadata {
	...
	if truncated {
		meta.Truncated = true
		meta.NextAction = "Raise --limit or start from a later --page-cursor to inspect more access policies"
	}
	return meta
}
